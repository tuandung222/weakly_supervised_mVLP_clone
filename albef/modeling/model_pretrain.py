...
from .model_utils import (
    get_pos_neg_sims,
    get_sims_from_mats_s2t,
    get_sims_from_mats_t2s,
)
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0):
        ...
        text_atts_neg = torch.index_select(
            text.attention_mask, dim=0, index=hard_text_index
        )
...
class ALBEF_fast(nn.Module):
    def __init__(
        ...
        self.text_encoder = BertForMaskedLM.from_pretrained(
            text_encoder, config=bert_config, ignore_mismatched_sizes=True
        )
    ...
    def model_part(self, part_name):
        ...
        if part_name == "txt_encoder":
            return [
                self.text_encoder.bert.encoder.layer[i] for i in range(0, fusion_layers)
            ] + [self.text_proj]
            ...
            return [
                self.text_encoder.bert.encoder.layer[i]
                for i in range(fusion_layers, num_layers)
            ]
    ...
    def forward(self, image, text, alpha=0):
        ...
        text_output = self.text_encoder.bert(
            input_ids, attention_mask=text.attention_mask, return_dict=True, mode="text"
        )
        text_embeds = text_output.last_hidden_state
        text_feat = F.normalize(self.text_proj(text_embeds[:, 0, :]), dim=-1)
        ...
        if get_rank() == 0 and (
            torch.isnan(loss_mlm) or torch.isnan(loss_itm) or torch.isnan(loss_ita)
        ):
            ...
            debug_info["single-modal"] = {
                "image": image_embeds,
                "text": text_embeds,
                "ita_sim": sim_i2t,
                "hard_img_index": hard_img_index,
                "hard_txt_index": hard_text_index,
            }
            debug_info["itm_out"] = {
                "itm_logit": vl_output,
                "labels": itm_labels,
                "last hidden states": torch.cat(
                    [output_pos.last_hidden_state, output_neg.last_hidden_state], dim=0
                ),
                "pos hidden": output_pos.hidden_states,
                "neg hidden": output_neg.hidden_states,
            }
            debug_info["mlm_out"] = {
                "mlm_logit": mlm_output.logits,
                "labels": labels,
                "hidden states": mlm_output.hidden_states,
            }
            torch.save(
                debug_info,
                "/remote-home/zjli/tmp_debug/debug_info_{:.4f}.pth".format(random_name),
            )
            raise ValueError
...
def get_default_config():
    ...
    print(f"The directory of the module is: {module_directory}")
    # The directory of the module is: /dscilab_dungvo/workspace/BA-PRE_THESIS/paper_clone_2/weakly_supervised_mVLP/albef/modeling

    config = yaml.load(
        open(
            # "/dscilab_dungvo/workspace/BA-PRE_THESIS/paper_clone_2/weakly_supervised_mVLP/albef/configs/pretrain_base_xlm-r_uni_freeze_vis.yaml",
            module_directory.replace("modeling", "configs/pretrain_base_xlm-r_uni_freeze_vis.yaml"),
            "r",
        ),
        Loader=yaml.Loader,
    )
    tokenizer = AutoTokenizer.from_pretrained("xlm-roberta-base", do_lower_case=True)
    config['bert_config'] = module_directory.replace("modeling", "configs/config_xlm-r.json")
    config['my_default_ckpt_path'] = module_directory.replace("albef/modeling", "pretrain_checkpoint.pt")
    # config['my_default_ckpt_path'] = torch.load(config['my_default_ckpt_path'], map_location=torch.device('cpu'))
    return config, tokenizer


class ALBEF_Stage1(nn.Module):
    config, tokenizer = get_default_config()
    ...
    def __init__(
        ...
        if "text_modeling" in config:
            ...
            if txt_teacher:
                ...
                print(
                    "learning from teacher: {}".format(txt_model_config["teacher_name"])
                )
                self.txt_teacher_model = model_class.from_pretrained(
                    txt_model_config["teacher_name"]
                )
        ...
        # NOTE: This is my inserted code for loading state dict from config['ckpt_path']
        if "my_default_ckpt_path" in config:
            msg = self.load_state_dict(torch.load(config["my_default_ckpt_path"])['model'])
            print(msg)
    ...
    def forward(
        ...
        text_trans=None,
        text_langs=None,
        text_trans_langs=None,
        mode="img-txt-contras",
        token_sim_mode="max",
        token_sim_k=3,
    ):
        ...
        if mode == "img-txt-contras":
            ...
            return self.forward_txt_tlm(
                text,
                text_trans=text_trans,
                text_langs=text_langs,
                text_trans_langs=text_trans_langs,
            )
        elif mode == "txt-txt-wla":
            return self.forward_txt_trans_weak_align(
                text,
                text_trans=text_trans,
                token_sim_method=token_sim_mode,
                sim_k=token_sim_k,
            )
    ...
    def forward_txt_trans(self, text, text_trans):
        ...
        trans_text_output = self.text_encoder.bert(
            text_trans.input_ids,
            attention_mask=text_trans.attention_mask,
            return_dict=True,
            mode="text",
        )
        trans_text_embeds = trans_text_output.last_hidden_state
        trans_text_feat = F.normalize(
            self.text_proj(trans_text_embeds[:, 0, :]), dim=-1
        )
    ...
    def forward_txt_trans_weak_align(
        self, text, text_trans, token_sim_method="max", sim_k=3
    ):
        ...
        hard_src_attn = torch.index_select(
            text.attention_mask, dim=0, index=hard_src_index
        )
        ...
        hard_tar_emb = torch.index_select(
            trans_text_embeds, dim=0, index=hard_tar_index
        )
        hard_tar_attn = torch.index_select(
            text_trans.attention_mask, dim=0, index=hard_tar_index
        )

        pos_sim_mats = torch.bmm(
            text_embeds, trans_text_embeds.permute(0, 2, 1)
        )  # batch src to target
        ...
        pos_weak_sims_s2t, pos_weak_sims_t2s = get_sims_from_mats_s2t(
            pos_sim_mats,
            text.attention_mask,
            text_trans.attention_mask,
            bi_direction=True,
            sim_method=token_sim_method,
            k=sim_k,
        )
        neg_weak_sims_s2t = get_sims_from_mats_s2t(
            neg_s2t_sim_mats,
            text.attention_mask,
            hard_tar_attn,
            bi_direction=False,
        ...
        neg_weak_sims_t2s = get_sims_from_mats_t2s(
            neg_t2s_sim_mats,
            hard_src_attn,
            text_trans.attention_mask,
            sim_method=token_sim_method,
        ...
        # loss computation
        wta_loss_s2t = torch.mean(
            torch.clamp(neg_weak_sims_s2t + 0.2 - pos_weak_sims_s2t, min=0)
        )
        wta_loss_t2s = torch.mean(
            torch.clamp(neg_weak_sims_t2s + 0.2 - pos_weak_sims_t2s, min=0)
        )
    ...
    def forward_txt_full(self, text, text_trans, text_langs=None, text_trans_lang=None):
        ...
        trans_input_ids, trans_labels = self.mask(
            trans_input_ids,
            self.text_encoder.config.vocab_size,
            trans_input_ids.device,
            targets=trans_labels,
            probability_matrix=probability_matrix,
        )
        ...
        masked_sequence_output = torch.masked_select(
            trans_text_embeds, lm_mask.unsqueeze(-1)
        ...
        if self.txt_teacher_model is not None:
            with torch.no_grad():
                ...
                teacher_logits = torch.masked_select(
                    teacher_logits, lm_mask.unsqueeze(-1)
                ).reshape(-1, teacher_vocab_size)
        ...
        if teacher_labels is not None:
            loss_distill = -torch.sum(
                F.log_softmax(half_prediction_scores, dim=-1) * teacher_labels, dim=-1
            )
            loss_distill = loss_distill.mean()
            masked_lm_loss = (
                1 - self.teacher_alpha
            ) * masked_lm_loss + self.teacher_alpha * loss_distill
    ...
    def forward_txt_tlm(self, text, text_trans, text_langs=None, text_trans_langs=None):
        ...
        input_ids, attn_mask, pos_ids, lang_ids = self.concate_pair(
            text, text_trans, text_lang=text_langs, trans_lang=text_trans_langs
        )
        # if get_rank()==0:
        #     print('text')
        #     print(text)
        ...
        full_text_output = self.text_encoder.bert(
            input_ids, attention_mask=attn_mask, return_dict=True, mode="text"
        )
        ...
        masked_sequence_output = torch.masked_select(
            full_text_embeds, lm_mask.unsqueeze(-1)
        ).reshape(-1, hidden_size)
    ...
    def forward_para_txt_full(self, text, text_trans):
        ...
        trans_probability_matrix = torch.full(
            trans_labels.shape, self.cond_mlm_probability
        )
        trans_input_ids, trans_labels = self.mask(
            trans_input_ids,
            self.text_encoder.config.vocab_size,
        ...
        s2t_mlm_output = self.text_encoder(
            encoder_embeds=text_embeds,
            attention_mask=text.attention_mask,
        ...
        t2s_mlm_output = self.text_encoder(
            encoder_embeds=trans_text_embeds,
            attention_mask=text_trans.attention_mask,
    ...
    def forward_img_txt_full(self, image, text, alpha=0):
        ...
        if get_rank() == 0 and (
            ...
            torch.save(
                debug_info,
                "/opt/tiger/tmp_debug/debug_info_{:.4f}.pth".format(random_name),
            )
            raise ValueError
    ...
    def concate_pair(self, text, text_trans, text_lang, trans_lang):
        ...
        for i in range(bs):
            ...
            concat_ids.append(
                torch.cat(
                    [
                        input_ids_a,
                        input_ids_b,
                        torch.ones(pad_size, dtype=torch.long, device=c_device)
                        * pad_id,
                    ]
                )
            )
            concat_attn.append(
                torch.cat(
                    [
                        torch.ones(size_a + size_b, dtype=torch.long, device=c_device),
                        torch.zeros(pad_size, dtype=torch.long, device=c_device),
                    ]
                )
            )
            concat_pos_ids.append(
                torch.cat(
                    [
                        pos_ids_a,
                        pos_ids_b,
        ...
        return (
            torch.stack(concat_ids),
            torch.stack(concat_attn),
            torch.stack(concat_pos_ids),
            torch.stack(concat_langs),
        )
